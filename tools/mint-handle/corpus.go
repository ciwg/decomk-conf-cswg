...
// scanLiteralDirs are the decomk-conf-cswg directories that own proquint handles.
// Missing directories are skipped so the tool still works in a partially
// migrated checkout or in focused tests.
//
// Intent: Treat the decomk-conf-cswg artifact corpus itself as the registry for
// TODO, TE, DR, and DI handles, avoiding a central sequence allocator. Source:
// DI-ruvop
...
// Filename grammar is KIND-HANDLE-SLUG.md where KIND is TODO, TE, DR, or DI.
...
// scanCorpus returns the set of handles currently owned by decomk-conf-cswg
...
// TODO files. Standalone DI files are already covered by their filenames, so
// scanning their DI-ID fields would count the same owner twice.
