# TODO-guvan: Mob Consensus Gui Desktop
...
Affects: `Makefile`, `decomk.conf`, `.devcontainer/Dockerfile`, `.devcontainer/devcontainer.json`, `TODO/TODO-guvan-mob-consensus-gui-desktop.md`, `TODO/TODO-fogup-mob-consensus-gui-stack.md`
Supersedes:
...
Affects: `Makefile`, `decomk.conf`, `TODO/TODO.md`, `TODO/TODO-guvan-mob-consensus-gui-desktop.md`

## Task 003 - Implement GUI Desktop for mob-consensus

- [x] guvan.1 Define the GUI desktop requirements for `mob-consensus` (window manager, browser/noVNC strategy, and required tooling) and capture accepted scope.
- [x] guvan.2 Add or confirm a `mob-consensus` repo context in `decomk.conf` that maps to `GUI_DESKTOP` behavior intentionally.
- [x] guvan.3 Replace the `GUIDesktop` placeholder Make target with concrete idempotent setup targets for GUI mode.
- [x] guvan.4 Keep `Block10` GUI-neutral by placing GUI-only apt installs in dedicated versioned GUI targets referenced only by GUI flows.
- [ ] guvan.5 Validate both `updateContent` and `postCreate` for GUI and non-GUI contexts, including manual target troubleshooting runs.
- [ ] guvan.6 Record verification evidence, update index/status, and close TODO-guvan.
